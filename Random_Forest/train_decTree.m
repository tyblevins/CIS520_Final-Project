...
[coeff,score,latent] = pca([X; X_test],'NumComponents',500);  %use test and train for PCA
% [coeff,score,latent] = pca([X],'NumComponents',1500);


%look at the coef 
[bigvalues, bigidx] = sort(abs(coeff(1:size(words_train,2),1)), 'descend');
...
% numParam = 10;
% pc_CV = (linspace(0.3,0.7,numParam));  %10 different values
% numFolds = 10;
...
%visualize
figure(2)
plot(PC_train(Y_train == 0,3),PC_train(Y_train == 0,4),'r*','markersize',5)
hold on;
plot(PC_train(Y_train == 1,3),PC_train(Y_train == 1,4),'bo','markersize',5)
legend('Male','Female')
xlabel('PC1')
ylabel('PC2')
...
mod = TreeBagger(500,PC_train,Y_train,'Method','classification','OOBPred','On','MinLeaf',10,'FBoot',1);
toc;
