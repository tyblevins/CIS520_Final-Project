...
raw_train   = dlmread('images_train.txt');
raw_test = dlmread('images_test.txt');
...
%compute mean image
imgAvg = zeros(100,100);
for i = 1:size(imgAvg,1)
    for j = 1:size(imgAvg,2)
        imgAvg(i,j) = mean(grayRaw(:,i,j));
    end
end
% imshow(uint8(imgAvg));

%remove mean from each picture then flatten
normGray = grayRaw - repmat(reshape(imgAvg,[1 100 100]), [size(grayRaw,1) 1 1]);
normGrayFlat = reshape(normGray, size(normGray,1),size(normGray,2)*size(normGray,3));  %flatten


[W,H] = nnmf(normGrayFlat,50);

numTr = size(raw_train,1);
